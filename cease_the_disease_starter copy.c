...
int is_safe(int x, int y) {
    return (x < TISSUE_SIZE) && (x >= 0) && (y < TISSUE_SIZE) && (y >= 0);
}
/**
...
{
    int xs[] = {-2,  0,  2, 2, 2, 0, -2, -2};
    int ys[] = {-2, -2, -2, 0, 2, 2,  2,  0};

    for (int i = 0; i < 8; i++) {
        if (is_safe(x + xs[i], y + ys[i])) {
            if (tissue_sample[x + xs[i]][y + ys[i]] == 'L')
                return true;
        }
...
{
    int xs[] = {-3, -3, -3, -2, -1, -1,  0,  1,  0,  0,  3, 3, 3, 2, 1, -1, 0, 1, 0, 0 };
    int ys[] = {-1,  0,  1,  0,  0, -3, -3, -3, -2, -1, -1, 0, 1, 0, 0,  3, 3, 3, 2, 1 };

    for (int i = 0; i < 20; i++) {
        if (is_safe(x + xs[i], y + ys[i])) {
            if (tissue_sample[x + xs[i]][y + ys[i]] == 'W')
                return true;
        }  
    }
    return false;
    ...
    for (int x = 0; x < TISSUE_SIZE; x++) {
        for (int y = 0; y < TISSUE_SIZE; y++) {
            if (tissue_sample[x][y] == 'I') {
                infected ++;
                if (canLeoEat(tissue_sample, x, y)) killed++;
                else if (canWillEat(tissue_sample, x, y)) killed++;
            }
        }
    }

    return (int) (((double) killed / (double) infected) * 100 + 0.5);
