...
int check_bound(int a, int b) {
    return (a < TISSUE_SIZE) && (a >= 0) && (b < TISSUE_SIZE) && (b >= 0);
}
...
int numberOfCharlesInRange(Tissue tissue_sample, int x, int y)
{
    ...
    for(int i=1;i<TISSUE_SIZE;i+=1){
        if(((tissue_sample[x+charles_patterns[0][0]*i][y+charles_patterns[0][1]*i].type==C&&check_bound(x+charles_patterns[0][0]*i,y+charles_patterns[0][1]*i))&&tissue_sample[x+charles_patterns[0][0]*i][y+charles_patterns[0][1]*i].health>=i)||
           ((tissue_sample[x+charles_patterns[1][0]*i][y+charles_patterns[1][1]*i].type==C&&check_bound(x+charles_patterns[1][0]*i,y+charles_patterns[1][1]*i))&&tissue_sample[x+charles_patterns[1][0]*i][y+charles_patterns[1][1]*i].health>=i)||
           ((tissue_sample[x+charles_patterns[2][0]*i][y+charles_patterns[2][1]*i].type==C&&check_bound(x+charles_patterns[2][0]*i,y+charles_patterns[2][1]*i))&&tissue_sample[x+charles_patterns[2][0]*i][y+charles_patterns[2][1]*i].health>=i)||
           ((tissue_sample[x+charles_patterns[3][0]*i][y+charles_patterns[3][1]*i].type==C&&check_bound(x+charles_patterns[3][0]*i,y+charles_patterns[3][1]*i))&&tissue_sample[x+charles_patterns[3][0]*i][y+charles_patterns[3][1]*i].health>=i)){
                count++;
...
int numberOfSakinaInRange(Tissue tissue_sample, int x, int y)
{
    /** TODO: implement this function, satisfying the description above. */
    int count=0;
    Pattern sa_patterns[4] = {{1, 1}, {-1, 1}, {1, -1}, {-1, -1}};
    
    for(int i=1;i<TISSUE_SIZE;i+=1){
        
        if(((tissue_sample[x+sa_patterns[0][0]*i][y+sa_patterns[0][1]*i].type==S&&check_bound(x+sa_patterns[0][0]*i,y+sa_patterns[0][1]*i))&&tissue_sample[x+sa_patterns[0][0]*i][y+sa_patterns[0][1]*i].health>=i)||
           ((tissue_sample[x+sa_patterns[1][0]*i][y+sa_patterns[1][1]*i].type==S&&check_bound(x+sa_patterns[1][0]*i,y+sa_patterns[1][1]*i))&&tissue_sample[x+sa_patterns[1][0]*i][y+sa_patterns[1][1]*i].health>=i)||
           ((tissue_sample[x+sa_patterns[2][0]*i][y+sa_patterns[2][1]*i].type==S&&check_bound(x+sa_patterns[2][0]*i,y+sa_patterns[2][1]*i))&&tissue_sample[x+sa_patterns[2][0]*i][y+sa_patterns[2][1]*i].health>=i)||
           ((tissue_sample[x+sa_patterns[3][0]*i][y+sa_patterns[3][1]*i].type==S&&check_bound(x+sa_patterns[3][0]*i,y+sa_patterns[3][1]*i))&&tissue_sample[x+sa_patterns[3][0]*i][y+sa_patterns[3][1]*i].health>=i)){
            //must implement the health function for the range of each bacteria
...
double testBacteria(Tissue tissue_sample)
{
    ...
    return ((double)killed/ (double)infected)*100;
}
